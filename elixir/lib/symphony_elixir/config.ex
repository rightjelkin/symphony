defmodule SymphonyElixir.Config do
  ...
  @tracker_sections ["linear", "github", "yougile", "memory"]
  ...
  def max_concurrent_agents_for_state(state_name) when is_binary(state_name) do
    ...
    by_state = get_in(opts, [:agent, :max_concurrent_agents_by_state]) || %{}
    normalized = normalize_issue_state(state_name)
    Map.get(by_state, normalized, get_in(opts, [:agent, :max_concurrent_agents]))
  end
  ...
  @spec codex_runtime_settings(Path.t() | nil, keyword()) :: {:ok, map()} | {:error, term()}
  ...
  def server_port do
    case Application.get_env(:symphony_elixir, :server_port_override) do
      ...
      _ -> get_in(validated_workflow_options(), [:server, :port])
    end
  end
  ...
  def validate! do
    with {:ok, _workflow} <- Workflow.current(),
      ...
      {:error, {:invalid_workflow_config, _} = reason} ->
        {:error, reason}
  ...
  defp tracker_config_module do
    case tracker_kind() do
      ...
      "yougile" -> SymphonyElixir.YouGile.Config
